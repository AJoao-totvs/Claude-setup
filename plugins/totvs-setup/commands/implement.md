...
- **ClickUp Task** - Check the plan's Metadata table for a ClickUp Task ID. If present, this task will be updated after implementation is complete.
...
## Phase 6: UPDATE CLICKUP (if task specified in plan)

**This phase is mandatory if the plan's Metadata table contains a ClickUp Task ID.** Skip only if the ClickUp Task field is "N/A" or absent.

If ClickUp MCP is available (tools prefixed with `mcp__clickup__`):

### 6.1 Update Task Status

Transition the task to the appropriate status (e.g., "in review", "done").
...
{If task was updated: "Updated ClickUp task: transitioned to {status}, added implementation comment." Otherwise: "No ClickUp task linked."}
