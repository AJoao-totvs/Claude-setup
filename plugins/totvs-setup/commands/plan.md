...
- **ClickUp Task**: If a ClickUp task ID is available in the conversation context — from a prior `/prime` command, user mention, or PRD — capture it. This is optional but should be included in the plan metadata when available so that `/implement` can update the task after completion.
...
| ClickUp Task | {task ID if available, or "N/A"} |
